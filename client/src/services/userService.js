...
export async function getUsersRoleCount() {
    try {
        const response = await axiosInterceptorInstance.get(`http://127.0.0.1:8000/api/user/user-role-count/`, {
            mode: 'no-cors',
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
...
export async function updateUser(userId, data) {
    try {
        console.log(data);
        const response = await axiosInterceptorInstance.put(`http://127.0.0.1:8000/api/user/${userId}/`, data
        );
        return response;
    } catch (error) {
...
export async function deleteUser(userId) {
    try {
        const response = await axiosInterceptorInstance.delete(`http://127.0.0.1:8000/api/user/${userId}/`, {
            method: 'DELETE',
        });
        return response;
